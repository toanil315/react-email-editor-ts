...
const createStyleString = (className: string, styles: BlockStyle) => {
  const regex = new RegExp(/[A-Z]/g);
  const kebabCase = (str: string) => str.replace(regex, (v: string) => `-${v.toLowerCase()}`);
  ...
  for (const item of Object.entries(styles.desktop)) {
  ...
  if (Object.keys(styles.mobile).length) {
    let mobile = '';
    for (const item of Object.entries(styles.mobile)) {
      if (item[1] && item[0] !== 'contentBackground') {
...
const createStyleTag = (list: Block[], styles = '', parentIndex?: string | number) => {
  const newBlockList = list.map((item, index) => {
    const newItem = item;
    newItem.styleConfig = createStyleString(
      parentIndex ? `${parentIndex}-${item.key}-${index}` : `${item.key}-${index}`,
      item.styles,
    );
    ...
    if (item.children?.length) {
      const { newBlockList: childrenList, styles: childrenStyles } = createStyleTag(
        item.children,
        styles,
        index,
      );
...
const createImageString = (imageConfig: Block) => {
  return `<div ${
    ...
    style="${imageConfig.contentStyleConfig?.desktop}">
        <img src="${imageConfig.src}" alt="${imageConfig.alt}" style="max-width:100%;${
...
const createTextString = (textBlock: Block) => {
...
const createHeaderString = (headerBlock: Block) => {
  return `<${headerBlock.type} ${
    headerBlock.styleConfig?.mobile ? `class="${headerBlock.styleConfig.className}"` : ''
  } 
...
const createButtonString = (buttonBlock: Block) => {
  return `<div ${
...
const createDividerString = (dividerBLock: Block) => {
  return `<div ${
    ...
    style="${dividerBLock.contentStyleConfig?.desktop}">
      <div ${
        dividerBLock.styleConfig?.mobile ? `class="${dividerBLock.styleConfig.className}"` : ''
      } 
      style="${dividerBLock.styleConfig?.desktop}"></div>
    </div>`;
};

const createSocialLinkString = (socialLinkBlock: Block) => {
  return `<div ${
...
const blockListToHtml = (blockList: Block[], bodySettings?: BodySettings) => {
  let content = '';
  blockList.forEach((item) => {
    if (item.key === 'column') {
      content += `<div ${item.styleConfig?.mobile ? `class="${item.styleConfig.className}"` : ''} 
        style="${item.styleConfig?.desktop};width:100%;display:block;">
          <table ${
            item.contentStyleConfig?.mobile ? `class="${item.contentStyleConfig.className}"` : ''
          } 
          style="width:100%;max-width:${bodySettings?.contentWidth}px;margin:0 auto;${
        item.contentStyleConfig?.desktop
      }">
    ...
    if (item.key === 'content') {
      content += `<td ${item.styleConfig?.mobile ? `class="${item.styleConfig.className}"` : ''} 
        style="width:${item.width}; ${item.styleConfig?.desktop}">${blockListToHtml(
        item.children,
      )}</td>`;
    }
